...
└──────────────────────────────────────────────────────┘
  ...
  ── OWASP NVD database ──────────────────────────────────
    Note     : each pipeline run's OWASP scan downloads the NVD database
               fresh (no shared pre-seeded cache) -- expect the first
               owasp-dc scan on a run to take longer than later stages.
